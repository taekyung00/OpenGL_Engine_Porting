...
// namespace
// {
// 	OpenGL::TextureHandle WhiteTexture() // just default texture
// 	{
// 		static OpenGL::TextureHandle white_texture = []()
// 		{
...
namespace CS230
{
	...
	Shape::Shape(Shape&& other) noexcept
		: vertexBuffer(other.vertexBuffer), vertexArrayObject(other.vertexArrayObject), textureHandle(other.textureHandle), primitivePattern(other.primitivePattern), vertexCount(other.vertexCount),
		  shapeShader(std::move(other.shapeShader))
	{
	...
	void Shape::Draw(Math::TransformationMatrix model_matrix, unsigned int color, float depth) const noexcept
	{
        ...
        // const auto camera_matrix_opengl = CS200::Renderer2DUtils::to_opengl_mat3(camera_matrix);
        // GL::UniformMatrix3fv(locations.at("uToNDC"), 1, GL_FALSE, camera_matrix_opengl.data());
